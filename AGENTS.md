...
[`README.md`](README.md) has the layout and the commands. [`docs/stack.md`](docs/stack.md) has every technical decision with the alternative that lost, so read it before proposing a different shape. [`PRODUCT.md`](PRODUCT.md) is who it is for; [`DESIGN.md`](DESIGN.md) holds the rules every screen follows and indexes the system files in `docs/design/system` and the feature design in `docs/design`; read the system file for what you are building before writing UI.
...
**Styling.** Tailwind v4 utilities over the Lymi tokens: `bg-plate`, `text-text-2`, `border-edge`. The token values are canonical as OKLCH variables in `apps/web/src/client/styles.css`, mirrored in the site's `styles.css`, and `DESIGN.md`'s frontmatter is generated from them; `pnpm check:design` fails on drift. A raw hex or a stock Tailwind color (`bg-slate-100`) in a component is a bug — it breaks dark mode, which is a separate warm palette rather than an inversion. Use logical direction utilities (`ms-`, `pe-`, `start-`, `text-start`) rather than physical ones (`ml-`, `pr-`, `left-`, `text-left`), so a right-to-left locale is a catalog and not a refactor; `inset-x-0` and centring with `left-1/2` are not directional and stay.

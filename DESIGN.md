...
A menu takes the shape of the machine too. On a desktop it is anchored to its trigger; on a touch device the same rows rise in a drawer from the bottom edge, where the thumb is, and swipe away. The rows keep their menu roles and arrow keys in both shapes, and a keyboard hint is left out of the drawer because there is no keyboard to hint at. `ResponsiveMenu` mirrors the Dropdown Menu anatomy, so a call site never asks which machine it is on; the rule is `(min-width: 768px) and (hover: hover) and (pointer: fine)`, read as the menu opens and held until it closes ([ADR 0017](docs/adr/0017-interface-primitives-are-shadcn-components-on-base-ui.md)).
...
A short list is picked from and a list of forty is searched, and both open the same way. `Select` and `Combobox` are the same box as every other control when closed. Open, a panel floats under the box: for `Select` the arrows walk the rows and typing a letter jumps to a name; for `Combobox` the panel starts with a search field and the names filter as you type. The rows are the control's own height, the chosen one leads with a check in a slot every row keeps, and the pointer's row is the row Enter picks. The panel is a popover, in the top layer, so the form under it does not move and the panel escapes every ancestor. That last part is what makes it work inside the phone's drawer: vaul translates the drawer to drag it, which makes the drawer the containing block for anything fixed inside it, and the top layer is the one place a transform cannot reach. Neither control is the platform `<select>`, whose list would be the one thing in a form the palette does not reach.

...
A card's picture is content, not decoration, so it follows the word's rules. In a picture mode it is the cue, alone on the plate, up to 46% of the viewport or 420 px tall. On reveal it shrinks to 220 px, the target takes the word's place under the rule, and the other text field follows as context; in a text mode the picture joins the context under the target at 180 px. `CardPicture` holds the picture's own shape from the first frame over a `plate-2` well, fades it in over 180 ms, and then drops the well, because a road sign on transparency would otherwise sit in a grey box. When it cannot load it says "Couldn't load the picture" and shows the description in its place, so the card can still be revealed and graded. The card page shows the picture and its description read-only, and says in words when picture review is waiting for a picture or a description.

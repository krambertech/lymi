...
Under `prefers-reduced-motion` the flame holds still, reveal, completion, card and toast crossfade with no travel, the list hover fades in on the row under the pointer instead of sliding, and the skeleton stops shimmering. The glow stays, because a glow is a state, not a movement.

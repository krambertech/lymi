...
The run lives in the chrome as a pill with no plate of its own, the flame and the number on the bar: on the rail's first line on desktop, and on the phone at the end of the top bar on Today and Library, before capture. Today also has the streak card, and the repeat is deliberate, because the bar stays the same on both tabs: the flame and the run, one plain line on where today stands (how many reviews are left to today's goal, or that the goal is reached or nothing is due), and the seven lights, where today's light shows how far the day has come. The whole card is the button, and it opens the same modal. Beside the number sits the flame, described under "The flame", which says whether the streak is alive. The pill opens the streak modal, a place in ADR 0017's sense: centred over the page on a desktop, and rising over the whole screen on touch, where a swipe down closes it. Its open state is `?streak=true` on any screen, so Back closes it and a reload keeps it. The modal holds the run with one plain line about today, today's attempts against the goal, tiles for the longest streak and the days reviewed, and the month. In the month a day that counted is ringed in amber and a faint band joins the days that kept the run, so a run is a length you can see; a nothing-due day carries the band without a ring.
...
Library lists the decks without a series first, exactly as before series existed. Each series follows under its own heading: the name, how many decks and cards it holds, and a menu with Edit series, Move up, Move down and Archive series. When any of its cards are due, a thin **Review this series** banner sits under the heading: the due count, how many decks the cards are in, and an arrow, the whole strip one link. It is sunk into a `plate-2` well with no edge, so it never reads as one more deck, and it steps aside when nothing is due. Its decks sit under the heading in the series' order. The learner's own decks can be dragged into, out of and along a series; a long press starts the drag on touch, Space on the keyboard, and Enter still opens the deck. A deck without a series keeps its place by creation date, so the loose grid takes a drop without making room, and its New deck tile stays when every deck is in a series, as the drop target out of one. The arrow keys can carry a deck out of its series as well as along it. The same moves exist without dragging: **New series** and **Archived series** in Library's menu, **Edit series** (a name and a checklist of decks with up and down arrows), and **Move to series** in a deck's menu. Archiving a series that has decks is the one archive that asks first, because the learner chooses whether the decks leave with it; everything else about a series undoes with a toast.

...
The streak counts consecutive learner-local days whose goal was satisfied. [The streak](docs/design/streak.md) covers how the number and the seven lights show it, and [the brand rules](docs/design/system/brand.md#flame) cover the flame. A day that ends without satisfying the goal becomes a rest day when the current run has had no other rest day in the six days before it: the run carries on through it without growing. Any other short day resets the current streak to zero, while the learner's review history remains intact. Rest is automatic and worked out on every read, so it applies to all history; misses from before the run began don't count, and nothing counts rest days down or warns about them. When zero cards are eligible, opening Lymi and letting it confirm that nothing is due protects the current streak without increasing it; not opening still makes the day missed.
...
A review runs until what it was opened for is empty: the whole day from the Review button, one deck, one section of a deck, one series, or a round. Only the day's draw stops, once, at the goal when it began below it, and Continue carries on from there to the end of the day. Crossing the goal in a deck, a section, a series or a round shows in the flame and never stops it. The header counts the goal while the day's draw is heading for it, and the stretch's own cards otherwise. The rules are in [ADR 0021](docs/adr/0021-a-review-runs-to-its-end-and-stops-once-at-the-goal.md).

Every ending is the same success screen, whether the stretch reached the goal, ran out, or the learner pressed X after grading: it shows today's total rolling up from the last screen, embers for what was added, and the streak ticking when the day just turned. While the day is open, **Continue** leads and says how many more reviews reach the goal; once it is satisfied, **Done** leads. **Nothing left in** a deck leaves the day open while other decks have cards; **You're done for today** means every deck ran out, and the day counts. **Review forgotten** is offered at any end with forgotten cards. Continuing is optional and cannot make a completed streak goal incomplete. When zero cards are due, the empty state protects but does not increase the streak and offers **Add cards**; adding alone does not count toward the review goal. Running out of cards while offline or after a failed refresh is not evidence that all useful reviews are finished.

Today also offers three rounds beside the day's draw: **New cards**, which starts unseen cards in the order the draw introduces them; **Forgotten today**, the same set as Review forgotten; and **Often forgotten**, cards forgotten at least 4 times in at least 6 reviews. A slipping card comes whether or not it is due and is graded like any review, so its schedule moves and the attempt counts toward the goal. Each card comes once per round, and a card already reviewed today waits for tomorrow.

## Brand Personality

Three words: warm, calm, quick.

The name is cut from lyhty, the Finnish word for lantern. The symbol is a storm lantern, the kind you carry. Its flame is the continuity of remembering: repetition keeps it alive, so it is out only when there is no streak and no review yet today, and the first review lights it again. What it shows in the signed-in product is in [the brand rules](docs/design/system/brand.md).
...
- All motion respects `prefers-reduced-motion`. The flame's flicker, its breath and spark after a review, and its rise at the goal each have a still alternative: the flame takes its new size and halo at once, and no spark plays.

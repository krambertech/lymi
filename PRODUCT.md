...
An active review is recomputed, never stored. The next card follows from the cards, today's reviews, and the date, so the same synced state gives the same next card after a reload, offline, on another device, or in another scope. A grade made on another device can change which card comes next, and an offline grade that reaches the server after a later grade of the same card does not count. The rules are in [ADR 0018](docs/adr/0018-the-review-queue-is-a-deterministic-weighted-draw.md).

When both due reviews and new cards are available, one ordinary attempt in five goes to a new card; if either group runs out, the other fills the remaining daily goal. The draw favours reviews still likely to be remembered and new cards added recently, while anything eligible can appear, and every fourth new card is the oldest one not yet started so a large deck keeps moving. These proportions are internal rules, not learner settings or promised quotas.

A forgotten card returns in the same direction a few cards later, and so does a card graded Hard before it is learned; each further slip brings it back after a longer gap, up to three returns in a day, and after that it waits until tomorrow. A card still being learned or relearned from an earlier day is mixed in the same way near the start of the next review. Once one direction of a card is reviewed, the other waits until tomorrow because the revealed answer would make it dishonest. A card introduces its directions one at a time in its deck's order, which starts with production by default.

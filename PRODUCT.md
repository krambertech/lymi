...
A review runs until what it was opened for is empty: the whole day from the Review button, one deck, one open section of a deck, one series, or a round. Only the day's draw stops, once, at the goal when it began below it, and Continue carries on from there to the end of the day. Crossing the goal in a deck, a section, a series or a round shows in the flame and never stops it. The header counts the goal while the day's draw is heading for it, and the stretch's own cards otherwise. The rules are in [ADR 0021](docs/adr/0021-a-review-runs-to-its-end-and-stops-once-at-the-goal.md).

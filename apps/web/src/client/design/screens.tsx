...
export const SCREENS: Entry[] = [
  {
    ...
    note: "The due card and the streak card share the top row: the lantern beside how many cards are due, one full-width Review button, and the run with its seven lights. Under them, the rounds as three tiles that keep their place when empty, then the decks with cards due when there is more than one deck. Rows are whole links, and no term appears, so the page never gives an answer away. Until the first review, Today is the getting started guide.",
    Demo: () => (

...
export type GoogleCloudTtsBindings = {
...
export interface GoogleSynthesisOptions {
  /** The `text:synthesize` body, without credentials. */
  body: unknown;
  request: Fetch;
  accessToken?: (() => Promise<string>) | undefined;
}

/** Call Cloud Text-to-Speech with the service account. Credentials are parsed only here. */
export async function synthesizeGoogleSpeech(
  env: GoogleCloudTtsBindings,
  { body, request, accessToken }: GoogleSynthesisOptions,
): Promise<Response> {
  ...
  if (!encoded) throw new GoogleSpeechError("Google Text-to-Speech returned no audio");
  try {
    return new Response(decodeBase64(encoded), { headers: { "Content-Type": "audio/mpeg" } });
  } catch (cause) {
    throw new GoogleSpeechError("Google Text-to-Speech returned invalid audio", { cause });
  }
}

function readCredentials(env: GoogleCloudTtsBindings): ServiceAccountCredentials {
  const raw = env.GOOGLE_CLOUD_TTS_CREDENTIALS?.trim();
  if (!raw) throw new GoogleSpeechError("Google Text-to-Speech is not configured");
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch (cause) {
    throw new GoogleSpeechError("Google Text-to-Speech credentials are invalid", { cause });
  }
  if (
    !parsed ||
    ...
    throw new GoogleSpeechError("Google Text-to-Speech credentials are invalid");
  }
...
async function serviceAccountAccessToken(
  ...
  if (
    ...
    throw new GoogleSpeechError("Google OAuth returned no access token");
  }
  const expiresIn =
...
async function signedAssertion(
  ...
  try {
    ...
    throw new GoogleSpeechError("Google Text-to-Speech private key is invalid", { cause });
  }
...
async function fetchWithRetry(request: Fetch, url: string, init: RequestInit): Promise<Response> {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      if (attempt === 1) {
        throw new GoogleSpeechError("Google Text-to-Speech could not be reached", { cause });
      }
      await pause(200);
      continue;
    }
    ...
    if (!retryable || attempt === 1) {
      throw new GoogleSpeechError(`Google Text-to-Speech request failed (${response.status})`, {
        status: response.status,
      });
    }
    await pause(200 * (attempt + 1));
  }
  throw new GoogleSpeechError(`Google Text-to-Speech request failed (${lastStatus})`, {
    status: lastStatus,
  });
}
...
async function boundedJson(response: Response): Promise<unknown> {
  ...
  if (length > MAX_RESPONSE_BYTES) {
    await response.body?.cancel();
    throw new GoogleSpeechError("Google Text-to-Speech returned an oversized response");
  }
  try {
    return await response.json();
  } catch (cause) {
    throw new GoogleSpeechError("Google Text-to-Speech returned invalid JSON", { cause });
  }
}

...
const INVITED = ["ada@lymi.test", "grace@lymi.test", "hedy@lymi.test", "mary@lymi.test"];
...
const signUp = (email: string, password = PASSWORD) =>
  post("/sign-up/email", { email, password, name: email.split("@")[0] });
...
async function openLastLink(email: string) {
  ...
  return auth().handler(new Request(url as string, { headers: { origin: PRODUCT_URL } }));
}
...
describe("creating an account with an email and a password", () => {
  it("grants nothing until the address is confirmed, then signs the learner in", async () => {
    ...
    // No session: the address is unproven, so sign-up hands out no cookie.

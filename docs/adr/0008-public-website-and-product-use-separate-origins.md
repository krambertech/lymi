...
- Redirect signed-in visitors from `lymi.app` to the product or share the product session across subdomains. Rejected: the website must remain deliberately accessible, and a static Open Lymi link lets the product origin handle authentication without broadening the session cookie's scope.

...
**Members catch up on request.** `deck_members.states_version` is the deck version the member's states last matched. `catchUpStates` finds the member's decks whose version is ahead, inserts every missing state for those decks, and copies the deck's version onto the membership in one batch. A member who is up to date costs one indexed read of their memberships.

**Where the catch-up runs.** Before `/api/decks`, `/api/series`, `/api/sections`, `/api/cards`, `/api/review`, `/api/stats` and `/api/exports`, before every MCP request, and before a push reminder counts. Joining a deck writes every state and sets the version directly.

**When a caught-up state counts as added.** A state counts as added when its card was created, or when the member joined if that was later. A state written at catch-up time would make a months-old card look new to the draw.

...
**Ids are made on the device.** `POST /api/cards`, `/api/cards/batch` and `/api/decks` take an optional `id` in `newId`'s alphabet. A create sent again with the same id returns what the first one made, with no second row or audit entry, and the server recognises it before any other check, so a card that landed is still found after its deck is archived. An id another learner holds is a 409. MCP's `add_cards` does not take one. A card or deck made offline can therefore be edited, archived, or have cards added to it before the server has seen it.
...
**Order and holds.** One flush runs at a time, under a Web Lock across tabs. Writes go oldest first, and before each one the grades made before it are sent, so the server sees one timeline. Offline, a send with no answer after 20 seconds, or a 401 stops the flush and keeps everything; a stalled send is tried again 30 seconds later. A list waits at most 2 seconds on the flush before it fetches anyway. A 408, 429 or 5xx backs off, from 2 seconds doubling to 5 minutes, and holds every later write to the same item, for up to a day; a failure no status explains gets five tries. After that, or on any other refusal, the write is dropped with every later write that needed what it would have created, and the learner is told once. A card add that the server skips as a duplicate is reported the same way.
...
- A queued write that fails its replay is dropped and reported rather than kept forever, so a server-side refusal or a day-long outage cannot block the queue for good.
- Mutations outside the outbox, such as settings, sections and series, now fail at once offline instead of pausing until the connection returns.

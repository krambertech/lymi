...
No tool grades reviews, whatever the scope. Every write runs through the service layer as actor `mcp` and appears in Activity.

## Tool output

Tools return the learner's text and the ids a follow-up call needs. Card output omits `createdBy`, `updatedAt`, `userId`, `normalizedTerm` and `audioKey`. A `ServiceError` reaches the assistant as its message. Any other error goes to the Worker log, and the assistant gets "Lymi could not finish this just now. Try again in a moment."

...
sequenceDiagram
  ...
  J-->>C: server HTML: deck name, count, owner, Join
  C->>J: POST Join
...
The join completes in the session hook because `safeProductReturnPath` in `apps/web/src/shared/origins.ts` only returns sign-in to the paths in `PRODUCT_PATHS`, and `/join` is not one. A hook also runs once per sign-in whatever the browser does with the callback page. A signed-in learner who opens the link gets the same page, and **Join** calls `join` directly. `join` refuses a member the owner removed; a named invitation clears that block.

## Order of work

Each step leaves the app deployable.

1. **Membership, read paths, and the response contract.** One migration: `deck_members` and the `card_states` index swap. `deckAccess` and `visibleDecks` in `services/decks.ts`. Route `listDecks`, `getDeck`, `listDeckCards`, `getCard`, `searchCards`, `reviewQueue`, `reviewHistory`, the stats queries, and the reminder query in `push-delivery.ts` through membership, filtering states on the caller's `user_id`. `pronunciationAudio` follows `getCard`; its `audio_key` write and audit row stay on the owner's card. Writes keep the owner filter. State fan-out for every member: `addCards`, `openDirections` on a deck direction change, and a new backfill in `updateCard` when a card's `directions` changes. `DeckSummary` and the deck response gain `role` and `owner: { name }`. New `services/members.ts` with `join`, `leave`, and `remove`; `join` creates states the way `openDirections` does, and refuses a member whose last row was removed by the owner. Tests: a member sees the deck, its cards, search results, and audio; grades their own state; cannot write a card; a deck direction change and a card direction change each reach the member; rejoin succeeds after leave and fails after remove; the owner's queue is unchanged.
2. **Invitations and the front door.** `deck_invitations`. The `user.create.before` hook allows an email in `ALLOWED_EMAILS`, an unrevoked email invitation, or a `lymi_join` cookie naming an unrevoked link. A `session.create.after` hook completes the join. Services: `createLink`, `revokeLink`, `inviteEmail`, `revokeInvitation`, `listMembers`, with API routes. A named invitation to a removed member clears the block. Tests: each gate, a revoked token, a repeated callback joining once, and a removed member re-admitted by name.
3. **The join page.** `decideOriginRoute` in `apps/web/src/server/origin-routing.ts` continues `/join/<token>` on the product surface while exact `/join` keeps redirecting to the public site; add the case to `origin-routing.test.ts`. `GET /join/:token` next to the API routes, returning server HTML with the Lymi tokens inlined and `og:title`, `og:description`, `og:image`. It shows the deck's name, card count, and owner, and no cards. Three states: live, turned off, deck archived. Signed out shows **Join with Google**; signed in shows **Join**, or **Open deck** for a member. `POST /join/:token` sets the cookie and redirects. Load `impeccable` and `ux-writing` first. Check the preview by fetching the route and by pasting the link into a chat.
...
5. **API and MCP.** Writes by anyone but the owner return 403 through every door. `list_decks`, `get_deck`, `get_card`, and `search_cards` return shared decks and their role. Update the OpenAPI descriptions and `docs/data-model.md`.
...
| PR | Steps | Gate |
| --- | --- | --- |
| 1. Membership | 1 | Unit tests for read paths, fan-out, rejoin, and write refusals. `pnpm test:e2e:chromium` for the owner's flows. |
...
PRs 2, 3, and 4 each depend only on PR 1.
...
- `decideOriginRoute` runs before Hono. It redirects exact `/join` to the public site and returns 404 for `/join/<token>` until step 3 adds the case. After that, Hono matches the route before the SPA fallback in `app.notFound`.
...
Editor and contributor roles in the interface, a contribution flow, email delivery for named invitations, personal notes or pausing on shared cards, a copy for a member who leaves, any view of a member's progress, transferring ownership, and a public deck directory. Trying the deck before joining, and showing any card on the join page, is described in [the proposal](../proposals/shared-decks.md#a-later-direction-try-the-deck-before-joining) and needs its own decision.

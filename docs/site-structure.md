...
| `lymi.app/decks/<slug>` | `apps/site` Worker | A published deck, rendered per request from D1: its name, publisher and **Add to Lymi**, which links to `my.lymi.app/add/<slug>`, over a spread of its cards; its sections as tiles in order, with every card in a view that opens over the page and at `#cards`; how adding and reviewing work beside a stack of eight random cards to turn over, with nothing saved; and its sources. An unknown slug is 404, a withdrawn or archived deck 410. ADR 0016. |

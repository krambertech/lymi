...
| `lymi.app/decks/<slug>` | `apps/site` Worker | A published deck, rendered per request from D1: its name and publisher, up to ten of its first section's cards to recall and mark Forgot or Knew it with nothing saved, how adding and reviewing work, its sections as the path a learner walks with each section's cards folded into it, its sources, and **Add to Lymi**, which links to `my.lymi.app/add/<slug>`. An unknown slug is 404, a withdrawn or archived deck 410. ADR 0016. |

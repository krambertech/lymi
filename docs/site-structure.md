...
| `lymi.app/decks/<slug>` | `apps/site` Worker | A published deck, rendered per request from D1: its name, publisher and **Add to Lymi**, which links to `my.lymi.app/add/<slug>`, over a spread of its cards; its sections as a contents list in order, with every card in a view that opens over the page and at `#cards`; how adding and reviewing work beside a hand of eight random cards, each turned once before it asks the visitor to keep going in Lymi, with nothing saved; and its sources. An unknown slug is 404, a withdrawn or archived deck 410. ADR 0016. |

...
| `lymi.app/decks/<slug>` | `apps/site` Worker | A published deck, rendered per request from D1: what it is, a few of its first cards to try, every card by section, its publisher and sources, and **Add to Lymi**, which links to `my.lymi.app/add/<slug>`. An unknown slug is 404, a withdrawn or archived deck 410. ADR 0016. |
